...
func (p *Provider) loadTCPRoutes(ctx context.Context, gatewayListeners []gatewayListener, conf *dynamic.Configuration, report *statusReport) {
	logger := log.Ctx(ctx)
	...
	for _, route := range routes {
		...
		report.tcpRoutes[ktypes.NamespacedName{Namespace: route.Namespace, Name: route.Name}] = gatev1alpha2.TCPRouteStatus{
